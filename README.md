...
> Updates are batched when possible, but many times need to be run as multiple queries. We recommend upsert when possible for better performance, as it can always be run as a single query.
