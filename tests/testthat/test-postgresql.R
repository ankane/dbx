...
conn <- dbxConnect(adapter="postgres", dbname="dbx_test")
...
dbExecute(conn, "CREATE TABLE orders (id SERIAL PRIMARY KEY, city text)")
...
test_that("insert works", {
  res <- dbxInsert(conn, "orders", new_orders[c("city")])
